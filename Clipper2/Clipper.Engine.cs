      ...
      else
      {
        ...
        *******************************************************************************/
        ...
        private static OutPt InsertOp(Point64 pt, OutPt insertAfter) {
            OutPt result = new OutPt(pt, insertAfter.outrec) { next = insertAfter.next };
        ...
        private void AddJoin(OutPt op1, OutPt op2) {
            if ((op1.outrec == op2.outrec) && ((op1 == op2) ||
              ...
              ((op2.next == op1) && (op2 != op1.outrec.pts))))
                return;
        ...
        private OutRec ProcessJoin(Joiner j) {
            ...
            if (or1 == or2 &&
                ((op1 == op2) || (op1.next == op2) || (op1.prev == op2)))
                return or1;
            ...
            for (; ; )
            {
                if (!IsValidPath(op1) || !IsValidPath(op2) ||
                  (or1 == or2 && (op1.prev == op2 || op1.next == op2)))
                    return or1;
                ...
                if (CheckDisposeAdjacent(ref op1, op2, or1))
                    continue;
                if (CheckDisposeAdjacent(ref op2, op1, or1))
                    continue;
                if (op1.prev.pt != op2.next!.pt &&
        ...
        private OutPt DoSplitOp(ref OutPt outRecOp, OutPt splitOp) {
            ...
            if ((Math.Abs(area2) >= 1) &&
                ((Math.Abs(area2) > Math.Abs(area1)) ||
                 ((area2 > 0) == (area1 > 0)))) {
                OutRec newOutRec = new OutRec() { idx = _outrecList.Count };
        ...
        private bool DeepCheckOwner(OutRec outrec, OutRec owner) {
            ...
            if (owner.splits != null)
                foreach (OutRec asplit in owner.splits!) {
                    ...
                    if (split == null || split.idx <= owner.idx || split == outrec)
                        continue;
