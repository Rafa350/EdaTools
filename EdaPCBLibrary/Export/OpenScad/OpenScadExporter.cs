    ...
    public class OpenScadGenerator {
        ...
        private void GenerateComponents(TextWriter writer, XmlNode node) {
            ...
            foreach (XmlNode elementNode in node.SelectNodes("elements/element")) {
                ...
                sb.Append("    package(");
                sb.AppendFormat("\"{0}:{1}\"", library, package);
                if (x != 0)
                    sb.Append(String.Format(CultureInfo.InvariantCulture, ", xPos = {0}", x));
                if (y != 0)
                    sb.Append(String.Format(CultureInfo.InvariantCulture, ", yPos = {0}", y));
                if (r != 0)
                    sb.Append(String.Format(CultureInfo.InvariantCulture, ", rot = {0}", r));
                if (!top)
                    sb.Append(", top = false");
                sb.Append(");");
